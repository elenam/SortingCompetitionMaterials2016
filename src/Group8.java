...
public class Group8 {
	...
	private static int[][] sort(int[][] toSort) {
		quickSort(toSort, 0, toSort.length - 1, new PointComparator());
		return toSort;
	}//end of sort
	...
	public static int partition(int[][] array, int first, int last, Comparator c){
		int[] x = array[last];
		int indexI = first - 1;
		for(int indexJ = first; indexJ < last; indexJ++){
			if (c.compare(array[indexJ], x) < 1){
				indexI++;
				swap(array, indexI, indexJ);
			}
		}
		swap(array, indexI + 1, last);

		return indexI + 1;
	}//end of partition

	
	public static int randomizedPartition(int[][] A, int first, int last, Comparator c){
		int pivotA = first + (int)(Math.random() * (last - first));
		int pivotB = first + (int)(Math.random() * (last - first));
		int pivotC = first + (int)(Math.random() * (last - first));
		int pivotIndex = -1;
			
		if(((c.compare(A[pivotA],A[pivotB])>-1)&&(c.compare(A[pivotA], A[pivotC])<1))||
				((c.compare(A[pivotA], A[pivotC])>-1)&&(c.compare(A[pivotA],A[pivotB])<1))){
			pivotIndex = pivotA;
		} else if(((c.compare(A[pivotB], A[pivotA])>-1)&&(c.compare(A[pivotB],A[pivotC])<1))||
				((c.compare(A[pivotB],A[pivotC])>-1)&&(c.compare(A[pivotB],A[pivotA])<1))){
			pivotIndex = pivotB;
		} else {
			pivotIndex = pivotC;
		}
		...
		return partition(A, first, last, c);
		
	}//end of randomizedPartition
