...
public class Group7 {
	...
	// makes recursive call to self and calls mergeRanges
	private static void mergeSort(double[][] values, int startIndex, int endIndex, int sortBy) {
		if ((endIndex - startIndex) >= 2) {
			int midPoint = (startIndex + endIndex) / 2;
			mergeSort(values, startIndex, midPoint, sortBy);
			mergeSort(values, midPoint, endIndex, sortBy);
			mergeRanges(values, startIndex, midPoint, endIndex, sortBy);
		}
	}
	
	//compares values, if distance is equal uses time as comparator
	private static boolean compareItems(double[][] values, int firstIndex, int secondIndex)
	{
		if(values[firstIndex][0] < values[secondIndex][0])
		{
			return true;
		}
		else if(values[firstIndex][0] == values[secondIndex][0])
		{
			if(values[firstIndex][1] < values[secondIndex][1])
			{
				return true;
			}
			return false;
	...
	// merges arrays and compares array values
	private static void mergeRanges(double[][] values, int startIndex, int midPoint, int endIndex, int sortBy) {
		int range = endIndex - startIndex;
		double[][] tempArray = new double[range][2];
		int firstIndex = startIndex;
		int secondIndex = midPoint;
		int copyIndex = 0;
		while (firstIndex < midPoint && secondIndex < endIndex) {
			if (compareItems(values, firstIndex, secondIndex)) {
				tempArray[copyIndex] = values[firstIndex];
				++firstIndex;
			} else {
				tempArray[copyIndex] = values[secondIndex];
				++secondIndex;
			}
			++copyIndex;
		}
		...
		for (int i = 0; i < range; ++i) {
			values[i + startIndex] = tempArray[i];
		}
	}
	...
	private static int[][] sort(int[][] toSort) {
		double[][] distanceArray = new double[toSort.length][2];
		
		// creates array of arrays containing distance and slot index (function time)
		for(int i = 0; i < toSort.length; i++)
		{
			double[] slot = new double[2];
			slot[0] = distance(toSort[i]);
			slot[1] = (double)i;
			distanceArray[i] = slot;
		}

		mergeSort(distanceArray, 0, distanceArray.length, 0);
			
		int[][] returnArray = new int[toSort.length][3];
		
		// adds items to returnArray
		for(int i = 0; i < distanceArray.length; i++)
		{
			returnArray[i] = toSort[(int)distanceArray[i][1]];
		}
